...
cmd_look (string str)
{
    ...
    if(!str || strlen(str) < 1 || str == "here"){/* we assume they wanted to look at the room */
	this_object ()->message (this_object ()->query_environment ()->query_long ());	/* need to clean this syntax */
	return 1;
    }else{/* he wanted to look at str */
	if(strlen(str) > 3 && str[..2] == "at "){/* strip at */
		str = str[3..];
	}
	if((item = this_object()->present(str)) || 
           (item = this_object()->query_environment()->present(str)) != nil ){/* inventory then environ, add in room look at stuff */
...
mixed cmd_wield(string str){
	...
	if(catch(call_other(thing, thing->query_wield_func()))){/* messages not handled */
...
mixed cmd_unwield(string str){
	...
	if(catch(call_other(thing, thing->query_unwield_func()))){/* messages not handled */
...
mixed cmd_equip(string str){
	...
	if(catch(call_other(thing, thing->query_equip_func()))){/* messages not handled */
...
mixed cmd_unequip(string str){
	...
	if(catch(call_other(thing, thing->query_unequip_func()))){/* messages not handled */
