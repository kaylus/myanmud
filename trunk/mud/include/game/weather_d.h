...
mapping get_date(){/* function to get month */
	mapping date;
	
	date = ([]);
	
	if(sscanf(GAME_TIME_RAW, "%s %s %d %d:%d:%d %d", date["weekday"], date["month"], date["day"], date["hour"], date["minute"], date["second"], date["year"]) != 7){/* received malformed game_time */
