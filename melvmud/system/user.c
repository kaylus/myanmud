...
void promote_to_wizard(){
    if(object_name(previous_object()) != MAKEWIZ_CMD)
    	return;
...
static void close(){
    ...
    player->query_environment()->room_tell(cap_name+" has fallen unconscious.\n");
}
...
void quit(){
	...
	/* If there's no associated player let's let anyone force us to quit.
       ...
       problem. */
    if(player && previous_object() != player)
    	return;
...
int linkdead(){
    return !query_ip_number(this_object());
}
