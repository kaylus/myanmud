...
# define NEW_INHERITABLE(prev, id, name, inherited) ({ prev, id, name, inherited, nil, nil })
# define NEW_NON_INHERITABLE(prev, id, name, inherited) ({ prev, id, name, inherited, 0 })
...
static create()
{
  ...
  LOGD->log("Creating objectd", "objectd");
  ...
  if (status()[ST_ARRAYSIZE] * 256 < status()[ST_OTABSIZE])
   ...
   LOGD->log("Objectd created.", "objectd");
}
...
void compile(string owner, object obj, string *source, string inherited...)
{
  ...
  if (source == nil) 
    /*LOGD->log(owner + " compiling object from string", "objectd");
  else*/
...
void compile_lib(string owner, string path, string *source, string inherited...)
{
  ...
  if (source == nil)
    /*LOGD->log(owner + " compiling lib from string", "objectd");
  else*/
...
void compile_failed(string owner, string path)
{
  ...
  LOGD->log(owner + " failed to compiled " + path, "objectd");
}
...
mixed include_file(string compiled, string from, string path)
{
  if (previous_object() != driver)
    ...
    /*LOGD->log("Include_file compiled="+compiled+" from="+from+" path="+path, "objectd");*/
    return path;/* soft return */
}
