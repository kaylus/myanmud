...
blocked_attacks = 0;
LOGD->log("pacified "+this_object()->query_Name()+" combat? "+(string)this_object()->query_in_combat(), "combat");

} 

int

 query_is_attacking(object ob)
{
...
int query_in_combat()
{				/* determine if this cat is in combat */
...
if (room->object_present(attackers[x]))
	    return attackers[x];
...
int query_offhand()
{				/* whether to launch an offhand attack */
 ...
 ******************/ 
void
do_hit(object attacker, object attacked, object weapon,
       string attacker_name, 
string target_name,
       string attacker_possessive)
{
    
string weapon_short, *verbage, *dam_mess, err;
...
attacker->message(MY_HIT);
    
attacked->message(T_HIT);
    
attacker->query_environment()->message(O_HIT, ( {
						    attacker, attacked}
					    ));
...
call_other(weapon, weapon->query_hit_func(), attacker,
			attacked);
 ...
 ******************/ 
void
do_miss(object attacker, object attacked, object weapon,
	string attacker_name, 
string target_name,
	string attacker_possessive)
{
...
attacker->message(MY_MISS);
    
attacked->message(T_MISS);
    
attacker->query_environment()->message(O_MISS, ( {
						     attacker, attacked}));
...
int
 ...
 hit_roll(object weapon, varargs object body)
{
 ...
 *******************************************/ 
int execute_attack(varargs int number)
{				/* number is number of attacks to execute, default 1 */
...
LOGD->log(attacker_name + " attacking " + target_name, "combat");
...
if (query_offhand()
	     && ta->query_environment()->object_present(target)) {
...
ta->message("%^CYAN%^hp: [ " + this_object()->query_health() + " / " +
		 this_object()->query_max_health() + " ]%^RESET%^\n\n");
 ...
 ***********************************/ 
void block_attack(varargs int number)
{				/* number is number of attacks to block, default 1 */
...
int unblock_attack(varargs int number)
{				/* and reversage */
 ...
 * attack that round          *
 ******************************/ 
int round_resolution()
{				/* called each round before attacks are launched */
...
if (!room->object_present(attackers[i]))
	    attackers -= ( {
			  attackers[i]}
	);			/* may be wrong */
...
if (sizeof(attackers) == 0)
	this_object()->message("Combat ends.\n");
...
{
    LOGD->log(this_object()->query_Name()+" stops attacking "+dead->query_Name(), "combat");
stop_attacking(dead);
...
if(round_resolution() && this_object()->query_health() > -1)
  execute_attack();
