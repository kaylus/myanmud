...
static void initialize(){
    send_message("DGD/Myan started at "+ctime(time())+".\n");
    ...
    send_message ("Loading the event daemon\n");
    compile_object(EVENT_D);

    send_message("Loading the start room\n");
    compile_object(START);

    send_message("Loading ships\n");
    compile_object("/system/sea_d");
    call_other("/system/sea_d", "???");
...
string path_read(string path){
    ...
    if(!previous_object()->valid_read(path))
    	return nil;
...
string path_write(string path){
    ...
    if(!previous_object()->valid_write(path))
    	return nil;
...
static void runtime_error(string error, int caught, int ticks){
	...
	player = this_user();
	if(player != nil)
	    player->catch_tell("The air quivers around you.\n");
