...
static nomask void input_to(varargs string func, int flag){
    object ob;
...
/* Hymael - adapted an uppercase and edited both for speed */
static nomask string uppercase(string str){
	int i;
	i = strlen(str);
	while(i--){
		if(str[i] >= 'a' && str[i] <= 'z')
...
static nomask string replace_string(string str, string pattern, string replace){
...
static nomask string wrap(string str, int len){
		string ret;
		int i, slen;
		if(len<1)
			len = 72;
...
/* Send a message to all players in the same room as the current one.
   ...
   cool.
*/
static nomask void say(string str, varargs object *excluded){
    object ob, env;
    object *contents;
    int i, sz;
    ...
    if(!contents || !(sz = sizeof(contents)))
    	return; /* should be impossible */

    for(i=0; i<sz; i++){
        if(!contents[i]->query_living())
        	continue;

		if(contents[i] == ob)
			continue;

		if(excluded && member_array(contents[i], excluded) > -1)
			continue;
...
static nomask int cat(string file){
    ...
    if((sz = sizeof(lines)) > MAX_CAT_LINES)
...
static nomask int copy_file(string oldfile, string newfile){
    ...
    if(file_exists(oldfile) < 1)
    	return 0;

    if(file_exists(newfile))
    	return 0;

    if(!valid_read(oldfile))
    	return 0;

    if(!valid_write(newfile))
    	return 0;
...
*/
static nomask int fail_msg(string str){
    this_player()->set_fail_msg(str);
    return 0; /* allow return fail_msg syntax */
}
...
nomask void do_heart_beat(){
    ...
    if(err){/* add in a logger sometime */
    	log_file(HB_LOG, "Error: "+err+"\nAt: "+ctime(time())+"\n");
		heart_beat_time = 0;
		return;
    }
...
/**************************
 * events follow, largely *
 * taken from kernel lib  *
 * -Hymael                *
 **************************/
 ...
 */
nomask void
_F_subscribe_event(object obj, string name, int subscribe)
{
...
static void subscribe_event(object obj, string name)
{
    ...
    obj->_F_subscribe_event(this_object(), /*owner,*/ name, 1);
}
...
static void unsubscribe_event(object obj, string name)
{
    ...
    obj->_F_subscribe_event(this_object(), /*owner,*/ name, 0);
}
...
{
	int handle;
	handle = call_other(this_object(), name, args);
}
...
static void event(string name, mixed args...)
{
    ...
    name = "evt_" + name;
    if(args){
    	args = ({ this_object() }) + args; /* so we have object that initiated call */
	}else{
		args = ({ this_object() });
	}
    sz = sizeof(objlist);
